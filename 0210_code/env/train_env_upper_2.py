...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _test_info=False, video_dir="./hrl_model_output/video_frames", _near_mode=False, _fov = 1.5 * np.pi, verbose=True, custom_trajectory=[], _self_center=False):
        """
        Basic Info:
        - Expansion Factor: 16
    ...
    def get_flow_velocity(self):
        """
        Return flow velocities at 5 sample points relative to the agent:
            P1: [x-12, y]
            P2: [x-12, y+8]
            P3: [x-12, y+4]
            P4: [x-12, y-8]
        ...
        Values out of the map are clipped to the boundary.
        """
        v_x = self.u_flow
        v_y = self.v_flow
        H, W = v_x.shape
        ...
        for dx, dy in offsets:
            px = np.clip(x + dx, 0, H - 1)
            py = np.clip(y + dy, 0, W - 1)

            vx_list.append(v_x[px, py])
            vy_list.append(v_y[px, py])
    ...
    def step(self, action):
        ...
        if self.include_flow:
            ...
            if self._plot_flow:
                ...
                self.subgoal_history.append(self.target_position_lc)
                self.lowlevel_action_history.append(self.action[:2])
    ...
    def _render_frame(self, _save=False, save_start_target_map=False):
        """
        Render a frame, save full image, and record per-step data:
        ...
        if _save:
            ...
            print(f"[SAVE] Full frame → {save_path}")

        # # ============ ③ 保存高层 Stream 图（轨迹 + sub-goal）===========
        # if hasattr(self, "subgoal_history") and len(self.subgoal_history) > 0:
        #     hl_path = os.path.join(self.video_dir, f"high_{self.frame_id:05d}.png")
        #     self._save_highlevel_stream_step(hl_path)

        # # ============ ④ 保存低层 Stream 图（轨迹 + 每步动作箭头）===========
        # if hasattr(self, "lowlevel_action_history") and len(self.lowlevel_action_history) > 0:
        #     ll_path = os.path.join(self.video_dir, f"low_{self.frame_id:05d}.png")
        #     self._save_lowlevel_stream_step(ll_path)

        # ============ ③ 保存每步数据 ============ 
        data_dir = os.path.join(self.video_dir, "step_data")
        os.makedirs(data_dir, exist_ok=True)

        step_data = {
            "position": np.array(self.agent_pos_history),
            "subgoal": np.array(getattr(self, "subgoal_history", [])),
            "action": np.array(getattr(self, "lowlevel_action_history", [])),
            "flow_x": self.u_flow.copy(),
            "flow_y": self.v_flow.copy()
        }

        np.savez(os.path.join(data_dir, f"step_{self.frame_id:05d}.npz"), **step_data)
    ...
    #         self.save_start_target_map()
    #         # self.save_flow_field_map()

    def plot_env(self, ax, sample_rate=10):
        """
        Plot the environment, including vorticity field, agent, targets, obstacles, and trajectory,
        without flow direction arrows. The vorticity is computed relative to the left inflow velocity.
        Coordinates are displayed in physical units (scale from grid units).
        """
        # -------------------------------
        ...
        if flow_x is not None and flow_y is not None:
            # 网格间距（物理单位）
            dx_phys = x_range / flow_x.shape[0]
            dy_phys = y_range / flow_y.shape[1]

            # 计算涡度
            dvdx = np.gradient(flow_y, dx_phys, axis=0)
            dudy = np.gradient(flow_x, dy_phys, axis=1)
            vorticity = dvdx - dudy
        ...
        ax.scatter(*target, color='magenta', label='Goal Position', zorder=5)
        ax.scatter(*current_target, color='darkorange', label='Sub-goal Position', zorder=5)

        # -------------------------------
        # Plot obstacles (cylinders)
        # -------------------------------
        for circle in circles:
            ax.add_patch(plt.Circle(circle["center"], circle["radius"], color='black', alpha=0.3, zorder=4))
        ...
        # -------------------------------
        start_region = plt.Circle(
            start_default, random_range,
            color='green', alpha=0.25, zorder=2, label='Start Region'
        )
        target_region = plt.Circle(
            target_default, random_range,
            color='yellow', alpha=0.25, zorder=2, label='Goal Region'
        )
        ...
        fov_angle = np.degrees(self._fov)
        angle_deg = np.degrees(agent_angle)
        center_angle = (angle_deg + 180) % 360  # agent forward direction
        start_angle = (center_angle - fov_angle / 2) % 360
        end_angle = (center_angle + fov_angle / 2) % 360

        # Wedge for obstacle sensing range
    ...
    def _save_highlevel_stream_step(self, save_path, color="orange"):
        if len(self.agent_pos_history) < 2:
            return
        
        fig, ax = plt.subplots(figsize=(6,6))
        self._draw_flow_background(ax)

        history = np.array(self.agent_pos_history)
        sub_raw = np.array(self.subgoal_history)

        # ====== 去重：仅保留sub-goal变化点 ======
        sub = []
        if len(sub_raw) > 0:
            sub.append(sub_raw[0])
            for i in range(1, len(sub_raw)):
                if not np.allclose(sub_raw[i], sub_raw[i-1]):
                    sub.append(sub_raw[i])
            sub = np.array(sub)

        # ====== 轨迹 ======
        ax.plot(history[:,0], history[:,1], color='green', lw=1.6)

        # ====== sub-goal 关键点（统一颜色）======
        if len(sub) > 0:
            ax.scatter(sub[:,0], sub[:,1], s=12, color=color, zorder=5)
    ...
    def _save_lowlevel_stream_step(self, save_path, scale=0.22, every=1):
        """背景流场 + 轨迹 + 动作箭头（更密集时仍干净）"""
        if len(self.lowlevel_action_history) == 0:
            return
        ...
        history = np.array(self.agent_pos_history)
        act     = np.array(self.lowlevel_action_history)

        ax.plot(history[:,0], history[:,1], color='green', lw=1.4)   # 更细更论文风

        for i in range(0, len(act), every):
            x,y = history[i]
            dx,dy = act[i] * scale
            ax.arrow(x,y,dx,dy,
                    head_width=0.16,          # 小箭头不遮轨迹
                    alpha=0.70,
    ...
    def _draw_flow_background(self, ax):
        """从plot_env中抽取的背景绘制模块,可复用到任意渲染中"""
        ...
        vmin, vmax = -1.0, 1.0
        vort_clip = np.clip(vorticity.T, vmin, vmax)

        x = np.linspace(0, x_range, flow_x.shape[0])
        y = np.linspace(0, y_range, flow_x.shape[1])
        X, Y = np.meshgrid(x, y)

        ax.pcolormesh(X, Y, vort_clip, cmap="seismic",
                    shading='auto', alpha=0.70,
                    vmin=vmin, vmax=vmax, zorder=1)

        # 障碍物保留
        for circle in self.circles:
            ax.add_patch(plt.Circle(circle["center"], circle["radius"], color='black',
                                    alpha=0.32, zorder=3))
    ...
    #     target_region = plt.Circle(
    #         target_default, random_range,
    #         color='yellow', alpha=0.25, zorder=2, label='Target Region'
    #     )
    #     ax.add_patch(start_region)
    #     ax.add_patch(target_region)
    ...
    def save_start_target_map(self):
        ...
        # 2️⃣ 保存流场数据
        flow_data_path = os.path.join(self.video_dir, f"flow_field_data_{self.flow_num}.npz")
        np.savez_compressed(
            flow_data_path,
            u_flow=self.u_flow,
            v_flow=self.v_flow,
        ...
        # 3️⃣ 保存元信息（转成 list/float 以便 JSON 可序列化）
        def to_list(v):
