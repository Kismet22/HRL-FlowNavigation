...
class foil_env:
    ...
    def _render_frame(self, _save=False, save_start_target_map=False):
        ...
        ax = self.ax
        ax.clear()
        self.plot_env(ax, mode=self.plot_mode)
        plt.draw()
    ...
    def plot_env(self, ax, mode="full"):
        """
        Plot the environment with two modes:
        - mode="full": full visualization (vorticity, agent, start/goal, FOV, obstacles, trajectory, custom traj)
        - mode="simple": simplified version (vorticity, agent, obstacles, trajectory, hover points, reference trajectory)
        ...
        mode : str
            "full" or "simple" to select visualization style
        """
        # -------------------------------
        # Extract environment variables
        # -------------------------------
        history = getattr(self, "agent_pos_history", [])
        start_default = getattr(self, "start_default", None)
        target_default = getattr(self, "target_default", None)
        random_range = getattr(self, "random_range", 0)
        ...
        im = ax.pcolormesh(X, Y, vorticity_clipped, cmap=cmap, norm=norm, shading='auto', alpha=0.7, zorder=1)
        ...
        if mode == "full":
            ...
            if current_target is not None:
                ax.scatter(*current_target, color='darkorange', label='Sub-target Position', zorder=5)
            ...
            #     ax.add_patch(plt.Circle(start_default, random_range, color='green', alpha=0.25, zorder=2, label='Start Region'))
            # if target_default is not None:
            #     ax.add_patch(plt.Circle(target_default, random_range, color='yellow', alpha=0.25, zorder=2, label='Goal Region'))

        elif mode == "simple":
            if target is not None:
                ax.scatter(*target, color='magenta', label='Navigation Target', zorder=5)
            # Hover points
            if hasattr(self, "hover_points") and len(self.hover_points) > 0:
                hx, hy = zip(*self.hover_points)
                ax.scatter(hx, hy, color='red', s=40, label='Hover Points', zorder=6)
            
            # Reference trajectory
            if hasattr(self, "custom_trajectory") and len(self.custom_trajectory) > 0:
                cx, cy = zip(*self.custom_trajectory)
                ax.plot(cx, cy, linestyle='--', color='orange', linewidth=2.0, label='Reference Trajectory', zorder=6)
            
            ax.legend(fontsize=20)
            # No grid, no start/goal, no FOV
            pass
        ...
        # ax.grid(False)  # No grid for either mode
        ...
        if mode == "full":
            # ax.set_title(f"Environment Visualization - Step: {getattr(self, 'step_counter', 0)} ({mode})")
            ax.set_title(f"Environment Visualization - Step: {getattr(self, 'step_counter', 0)}")
    ...
    #     sample_rate : int
    #         Downsampling rate for plotting flow field arrows (unused here, retained for interface consistency).
    #     """
    ...
    #     # -------------------------------
    #     # Plot vorticity heatmap
    #     # -------------------------------
    #     vmin, vmax = -1.0, 1.0  # 可根据实际流场调节范围
    #     vorticity_clipped = np.clip(vorticity.T, vmin, vmax)
    #     cmap = plt.cm.seismic  # 红蓝表示旋转方向
    #     norm = mcolors.Normalize(vmin=vmin, vmax=vmax)
