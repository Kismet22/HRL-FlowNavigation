...
def train(model_id, render_mode=None, checkpoint_path_high=None, checkpoint_path_low=None):
    ...
    if _is_head:
        if obs_task:
            ...
            # random_range = 56
            _start = np.array([float(180), float(64)])
            _target = np.array([float(64), float(64)])
            random_range = 32
        else:
        ...
        if theta_mode:
            ...
            if not _near_mode:
                if obs_task:
                    env_name = f'train_5_dim_{state_dim_mode}_small_scale_r_{r_low}_{_fov_angle}'
                else:
                    env_name = f'train_5_dim_{state_dim_mode}_no_obs'
            else:
                env_name = f'Flow_env_Upper_dim_{state_dim_mode}_obstacle_small_scale'
    ...
    _forward_only=_is_head, _obstacle_avoid=True, _theta_mode = theta_mode, _is_switch=switch_mode, u_clip=clip_max, v_clip=clip_max, _near_mode=_near_mode, _fov=_fov, _self_center=self_center)
    ...
    while hl_time_step <= hl_max_training_timesteps:
        ...
        while not done and t < max_ep_len:
            ...
            if _is_head:
                lc_target_x, lc_target_y = compute_subgoal_forward(pos=agent_position, angle=high_action, r=r_low)
            else:
                if not theta_mode:
                    vec = high_action
                ...
                lc_target_x, lc_target_y = compute_subgoal(pos=agent_position, angle=high_action, r=r_low)
            ...
            if prev_high_action is not None:
                ...
                smoothness_penalty = -5 * high_action_diff/np.pi
