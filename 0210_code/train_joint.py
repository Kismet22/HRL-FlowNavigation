...
def train(model_id, render_mode=None, checkpoint_path_high=None, checkpoint_path_low=None):
    ...
    ####### Training environment variable settings #######
    ...
    hl_max_training_timesteps = int(2.1e5) 
    # hl_max_training_timesteps = int(1.8e5)  
    ...
    if _is_head:
        ...
        if theta_mode:
            ...
            if not _obs_mode:
                if lc_train:
                    env_name = f'train_4_dim_{state_dim_mode}_lc_train_no_obs'
                else:
                    env_name = f'train_4_dim_{state_dim_mode}_no_obs'
            else:
                env_name = f'train_4__dim_{state_dim_mode}_obstacle'

    env = train_env_upper_2.foil_env(
        args_1,
        max_step=max_steps, start_center=_start, target_center=_target,
        _random_range=random_range, _init_flow_num=flow_seed, _pos_normalize=_is_normalize, _is_test=test_mode,
        _state_dim=state_dim_mode, _is_random=true_random, _set_random=random_seed, _switch_range=switch_range,
        u_range=input_max, v_range=input_max,
        _forward_only=_is_head, _obstacle_avoid=True,
        _theta_mode=theta_mode, _is_switch=switch_mode,
        u_clip=clip_max, v_clip=clip_max, _near_mode=_obs_mode
    )
    ...
    # check if log file exists and read last line
    ...
    if icm_mode:
        ppo_agent = ICM_PPO(
            ...
            continuous_action_output_bias=action_output_bias, icm_alpha=100)
    else:
    ...
    while hl_time_step <= hl_max_training_timesteps:
        ...
        # Save previous valid state (used for difference/reward)
        ...
        # === Episode statistics variables ===
        current_ep_return = 0
        lc_current_ep_return = 0 if lc_train else None
        t = 0
        done = False

        # === High-level reference angle ===
        ...
        while not done and t < max_ep_len:
            # === High-level decision: update every max_ll_steps ===
            # max_ll_steps = 20
            max_ll_steps = 5
            high_state = state
            high_action = ppo_agent.select_action(high_state)     # high-level output target angle
            high_action = np.clip(high_action, env.low, env.high) # limit action range
            hl_time_step += 1

            # === Convert angle to subgoal coordinates ===
            agent_position = env.agent_pos
            ...
            if obs_task:
                r = 3
            if _is_head:
                lc_target_x, lc_target_y = compute_subgoal_forward(pos=agent_position, angle=high_action, r=r)
            else:
                if not theta_mode:
                    vec = high_action
                ...
                lc_target_x, lc_target_y = compute_subgoal(pos=agent_position, angle=high_action, r=r)
            ...
            while ll_step < max_ll_steps:
                ...
                # Construct low-level state: add relative position of subgoal
                ...
                # === Determine if low-level completes subgoal, fails, or reaches max steps ===
                ...
                if lc_train:
                    ...
                    # === Angle penalty (encourage smoothness) ===
            ...
            # === Mixed training: based on subgoal proximity ===
            ...
            # === Mixed training: based on subgoal proximity ===
            # cumulative_high_reward += 2 * max((relative_dist_to_subgoal - dist_to_subgoal), -5)
            # current_ep_return += cumulative_high_reward
            ...
            # log in logging file, record average return over multiple episodes
...
if __name__ == '__main__':
    ...
    # required arguments: start ID and training times
    ...
    # optional arguments: specify checkpoint paths for high-level and low-level models
    ...
    # training delayed start prompt
    total_delay = 1  # can be increased if needed, e.g., 10
    interval = total_delay if total_delay < 10 else 10

    for remaining in range(total_delay, 0, -interval):
        print(f"Program will start in {remaining} seconds...")
        time.sleep(interval)
