...
// 外力对象
class ForceInfo {
  PVector f_external;  // 外力（全局坐标系）
  float m_external;    // 外力矩
  PVector f_fluid;     // 流体力
  float m_fluid;       // 流体力矩

  ForceInfo(PVector f_ext, float m_ext, PVector f_fl, float m_fl) {
    f_external = f_ext;
    m_external = m_ext;
    f_fluid = f_fl;
    m_fluid = m_fl;
...
class Body {
  ...
  float phi=0, dphi=0, dotphi=0, ddotphi=0; // 定义角度;角度增量;角度的一阶导;角度的二阶导
  float mass=1, I0=1, area=0; // 质量;转动惯量;面积(由顶点 coords 计算)

  ArrayList<PVector> coords=new ArrayList<PVector>(); //刚体顶点列表
  int n; //顶点数量
  boolean convex=false; // 是否凸多边形
  OrthoNormal orth[]; // 各边的法向量
  

  // pfree true -> false,强制智能体不作旋转
  boolean pressed=false, xfree=true, yfree=true, pfree=true; // 自由度 x y spine

  // dotxc代表速度
  PVector xc, dxc=new PVector(), dotxc=new PVector(), ddotxc=new PVector(); // 空间位置X;增量;一阶导;二阶导
  ...
  // Body初始化;放置位置
  // 创建示例:Body b = new Body(100, 200, window, new PVector(2, -1));
  Body( float x, float y, Window window ) {
    this.window = window;
    xc = new PVector(x, y);
  }
  // Body(float x, float y, Window window, PVector initVelocity) {
  //     this.window = window;
  //     xc = new PVector(x, y);
  //     if (initVelocity != null) dotxc = initVelocity.copy();
  // }
  ...
  void end(Boolean closed) {
    ...
    if (n>4) {
      ...
      // box = new Body(xc.x, xc.y, window, null); // initVelocity 可以传 null
      box.add(mn.x, mn.y);
      box.add(mn.x, mx.y);
      box.add(mx.x, mx.y);
      box.add(mx.x, mn.y);
  ...
  void setVelocity(PVector vel) {
      if (vel != null) {
          println("Setting body velocity to: " + vel);
          dotxc = vel.copy();  // 复制，避免引用外部对象
      } else {
          println("Input velocity is null, cannot set velocity!");
      }
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // 速度 = 平移速度 + 角速度 × 半径向量 平动和转动结合后的真实的速度
  ...
  ForceInfo react_1(BDIM flow, PVector force, float moment) {
    // 流体力
    PVector f_fluid = pressForce(flow.p).mult(-1);
    float m_fluid = pressMoment(flow.p) * (-1);

    // 外力
    PVector f_external = bodyToGlobalForce(force);
    float m_external = moment;

    // 总力
    PVector f_total = f_external.copy().add(f_fluid);
    float m_total = m_external + m_fluid;

    // 更新刚体状态
    react(f_total, m_total, flow.dt);

    // 返回四个值
    return new ForceInfo(f_external, m_external, f_fluid, m_fluid);
  }
...
//   // 新增带速度的构造函数
//   EllipseBody(float x, float y, float _h, float _a, Window window, PVector initVelocity) {
//     super(x, y, window, initVelocity);  // 这里调用的是 Body 的新构造函数
//     h = _h;
//     a = 1.0/_a;
