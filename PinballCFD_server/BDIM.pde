...
*********************************************************/
// 一个二维不可压 Navier–Stokes 流体求解器，使用 BDIM（Boundary Data Immersion Method）将“给定运动的刚体”作为光滑沉浸边界嵌入到欧拉网格中
// 流体仿真定义
class BDIM{
  ...
  // 网格大小
  int n,m; 
  float t=0, dt, nu, eps=2.0;//起始时间;时间间隔;nu:流体粘度系数、eps：光滑参数
  ...
  // u0 → 上一步速度场或临时存储
  // ub → 刚体速度在网格上的投影
  // del, del1 → BDIM 光滑卷积核 / 权重函数
  // wnx, wny → 网格点上的法向权重（与刚体边界相关）
  // distance → 网格到刚体表面的距离场
  // rhoi → 与体积力或惰性耦合相关
  // c → 系数场，用于更新速度或压力
  // p → 压力场
...
*/
    R.plusEq(PVector.mult(g,dt)); //加外力
  ...
  void get_coeffs( Body body ){
    get_dist(body); // 网格到刚体距离
    get_del(); // 0阶光滑卷积核
    get_del1(); // 1阶光滑卷积核
    get_ub(body); // 刚体速度在网格上的投影
    get_wn(body);  // 网格法向
  }
