...
void draw() {
  ...
  // 获取当前位置、速度和表面压力
  ForceInfo info = test.ellipseForceInfo;
  pos = test.pos;
  vel = test.vel;
  surfacePressures = test.surfacePressures;
  // String state_cn = multy_state(pos, vel, surfacePressures);
  String state_cn = multy_state(pos, vel, surfacePressures, info);
...
public class serverHandler {
    ...
    public String reset(String actionInJson) {
        ...
        // 初始流场编号
        float flow_init = input_object.getFloat("flow_num");

        // 可选初始速度
        Float init_vx = input_object.containsKey("init_vx") ? input_object.getFloat("init_vx") : null;
        Float init_vy = input_object.containsKey("init_vy") ? input_object.getFloat("init_vy") : null;

        println("Reset_simNum " + simNum);

        // -------------------------
        // 3. 调用原 setUpNewSim，重置仿真环境
        setUpNewSim(simNum, init_x, init_y, flow_init);

        // -------------------------
        // 4. 单独修改椭圆体速度
        if (init_vx != null && init_vy != null && test != null) {
            // 打印接收到的速度
            println("Received initial velocity for ellipse: vx=" + init_vx + ", vy=" + init_vy);
            
            PVector newVel = new PVector(init_vx, init_vy);
            test.setEllipseVelocity(newVel);  // 会打印速度
        } else {
            println("No initial velocity provided or test is null.");
        }
        ...
        try {
            ...
            println("[Error] state do not refresh: " + e);
        }
...
void setUpNewSim(int runNum, float init_pos_x, float init_pos_y, float init_flow) {       
  ...
  test = new Pinball(resolution, Re, gR, theta, xi0, xi1, xi2, tStep, xLengths, yLengths, true, init_pos_x, init_pos_y, init_flow);
...
void setUpNewSim(int runNum) {       
  // 如果没有传入坐标，则使用默认的坐标值
  // setUpNewSim(runNum, 0.0f, 0.0f, 1.0f, null, null);
  setUpNewSim(runNum, 0.0f, 0.0f, 1.0f);
}

public String multy_state(
    PVector pos,
    PVector vel,
    ArrayList<Float> surfacePressures,
    ForceInfo info
    ...
    // -----------------
    // forces (如果 info 不为 null)
    if (info != null) {
        // 打印调试
        // println("info.f_external.x = " + info.f_external.x);
        // println("info.f_external.y = " + info.f_external.y);
        // println("info.m_external = " + info.m_external);
        // external force
        multy_state_json.put("f_ext_x", info.f_external.x);
        multy_state_json.put("f_ext_y", info.f_external.y);
        multy_state_json.put("m_ext", info.m_external);

        // fluid force
        multy_state_json.put("f_fluid_x", info.f_fluid.x);
        multy_state_json.put("f_fluid_y", info.f_fluid.y);
        multy_state_json.put("m_fluid", info.m_fluid);

        // total force
        multy_state_json.put("f_total_x", info.f_external.x + info.f_fluid.x);
        multy_state_json.put("f_total_y", info.f_external.y + info.f_fluid.y);
        multy_state_json.put("m_total", info.m_external + info.m_fluid);
    }
