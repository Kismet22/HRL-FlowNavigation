...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _test_info=False, video_dir="./hrl_model_output/video_frames", _near_mode=False):
        """
        Basic Info:
        - Expansion Factor: 16
    ...
    def step(self, action):
        ...
        # ------------------ 统一障碍物选择 ------------------
        if self._near_mode:
            obstacles = [dict(center=self._roll_center, radius=self.window_r/2)]
        else:
            obstacles = self.circles[:3]
        ...
        for circle in obstacles:
            ...
            if angle <= (_fov / 2) or self._near_mode:
        ...
        if len(_barricade_states) == 0:
            ...
            _real_dis_2_barricade = closest_distance
        else:
            ...
            if self._pos_normalize:
                ...
                # 保留 clip 前后的两种距离
                _dis_2_barricade = min(closest_distance, self.max_detect_dis)   # 用于输入
                _real_dis_2_barricade = closest_distance                       # 真实距离
            else:
                closest_dx = raw_dx
                closest_dy = raw_dy
                _dis_2_barricade = closest_distance
        ...
        if self.is_out_of_bounds():
            _out_bound, _truncated = True, True
            _reward = -100 if self._near_mode else -500
            ...
            _out_bound, _truncated, _reward = True, True, -500
            print(colored(f"**** Episode Finished At: {self.step_counter} **** Invalid state_1 detected (NaN/Inf).", 'red'))

        elif self._near_mode and _real_dis_2_barricade > 48:
            _out_bound, _truncated, _reward = True, True, -100
            print(colored(f"**** Episode Finished At: {self.step_counter} **** Too Far.", 'red'))

        elif self.is_in_circle(self.circles):
            self._collision_count += 1
            if self._collision_count > 0:  # end immediately
                _collide, _truncated = True, True
                _reward = -100 if self._near_mode else -250
                print(colored(f"**** Episode Finished At: {self.step_counter} **** Too Many Collisions.", 'yellow'))
            else:  # first hit (forbidden move)
            ...
            _time_out, _truncated = True, True
            _reward = -100 if self._near_mode else 0
        ...
        # ========== Normal Reward Calculation ==========
            ...
            roll_switch_pun  = -100 * abs(direction - self.old_direction) * abs(self.old_direction) * abs(direction)
            border_penalty   = -50 * np.sum(1.0 - state_border)

            # Reward composition by observation_dim
            reward_map = {
                (14, 6):  lambda: time_punish + target_reward,
                (16, 8):  lambda: time_punish + target_reward + avoid_penalty,
                (18, 10): lambda: (time_punish + target_reward + avoid_penalty + angle_reward + roll_switch_pun
                                if self._obstacle_avoid else
                                time_punish + target_reward + border_penalty),
                (20,):    lambda: time_punish + target_reward + avoid_penalty + border_penalty,
                (22,):    lambda: time_punish + target_reward + 2 * avoid_penalty + angle_reward + roll_switch_pun + border_penalty,
                (25,):    lambda: (time_punish + target_reward + avoid_penalty + angle_reward + border_penalty + roll_switch_pun
                                if d >= near_threshold else
                                time_punish + 5 * target_reward),
            }

            # 选择 reward 计算方式
            _reward = None
            for dims, func in reward_map.items():
                if self.observation_dim in dims:
                    _reward = func()
                    break
    ...
    def generate_start_and_target_pair(self, circle_center=(220, 64), circle_radius=32,boundary_margin=16.0,min_angle_deg=90, max_angle_deg=170, max_tries=1000):
        ...
        - 起点一象限 -> 终点三象限
        - 起点四象限 -> 终点二象限
        - 夹角限制: min_angle_deg <= angle <= max_angle_deg
        """
        ...
        for _ in range(max_tries):
            ...
            # 夹角限制: 大于 min_angle，小于 max_angle
    ...
    def reset(self, _init_vx=None, _init_vy=None):
        self.safe_range_flag = False
        self.frame_id = 0
        self._roll_center = np.array([0.0, 0.0])
        ########################################################################################
        ...
        if not self._is_fixed_target and not self._is_fixed_start:
        ...
        action_json = {
            ...
            "init_x": agent_init_x, 
            "init_y": agent_init_y, 
            "flow_num": float(flow_init)
        }

        # 只有当 vx, vy 不为空时才传
        if _init_vx is not None:
            action_json["init_vx"] = _init_vx
        if _init_vy is not None:
            action_json["init_vy"] = _init_vy
