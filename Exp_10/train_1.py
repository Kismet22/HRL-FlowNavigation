...
def train(model_id, render_mode=None, checkpoint_path=None):
    ...
    env_name = f'RL_Near_state_dim_{state_dim_mode}'
    ...
    env = train_env_upper_2_1.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _random_range=_random_range, _init_flow_num=flow_seed, _pos_normalize=_is_normalize, _is_test = test_mode, _state_dim=state_dim_mode, 
    _is_random = true_random, _set_random=random_seed, _switch_range = _switch_range, u_range=input_max, v_range=input_max, 
    _forward_only=False, _obstacle_avoid=True, _theta_mode = True, _is_switch=switch_mode, u_clip=clip_max, v_clip=clip_max, _obstacle_mode=True)
