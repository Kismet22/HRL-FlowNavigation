...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _forward_only = False, _theta_mode=False, _pos_normalize=True, _obstacle_avoid=False, _obstacle_mode=False, _is_switch=False, _is_test=False,
             _random_range = 56, _flow_range=16, _switch_range = 12, _init_flow_num=0,  _is_random = True, _set_random=0,
             _state_dim=18, u_range=20.0, v_range=20.0, w_range=5.0, u_clip=20.0, v_clip=20.0, w_clip=5.0,
             _include_flow=False, _plot_flow=False, _proccess_flow=False,        
             video_dir="./hrl_model_output/video_frames"):
        """
        Basic Info:
        - Expansion Factor: 16
    ...
    def generate_two_obstacles_in_rect(self, x_range=(164, 232), y_range=(48, 80), radius=8, max_tries=1000, n=1):
        """
        在矩形区域内生成两个不相交的障碍物
        - x_range, y_range: 矩形范围
        - radius: 障碍物半径
        - max_tries: 最大尝试次数
        """
        obstacles = []
        ...
        while len(obstacles) < n and tries < max_tries:
            ...
            for obs in obstacles:
                if np.linalg.norm(candidate - np.array(obs["center"])) < 2 * radius:
                    overlap = True
                    break

            if not overlap:
                obstacles.append({"center": (float(x), float(y)), "radius": radius})

        if len(obstacles) < n:
            raise RuntimeError(f"Failed to generate 2 obstacles after {max_tries} tries.")
    ...
    def reset(self):
        ...
        if not self._is_fixed_target:
            print("Random Target")
            tx = np.random.uniform(130, 146)
            ty = np.random.uniform(48, 80)
            self.target_position = np.array([tx, ty])
            self.target_position_lc = self.target_position

        if not self._is_fixed_start:
            print("Random Start")
            sx = np.random.uniform(262, 278)
            sy = np.random.uniform(48, 80)
            self.start_position = np.array([sx, sy])
        ########################
        ...
        if not self._is_fixed_target and not self._is_fixed_start:
            ...
            if self._obstacle_mode:
                ...
                self.obstacles = self.generate_two_obstacles_in_rect(x_range=(164, 232), y_range=(48, 80), radius=8)

        self.heading = 0.0 if self.target_position[0] > self.start_position[0] else np.pi # 0:x轴正方向
        ...
        # ########################################################################################
        # # Obstacle sensing in global coordinates (360°)
        # _barricade_states = []
        ...
        #     vec_to_obs = obstacle_pos - agent_pos
        #     center_distance = np.linalg.norm(vec_to_obs)
        ...
        #     # Edge point in world coordinates
        #     unit_vec_to_obs = vec_to_obs / center_distance
        #     edge_point_world = obstacle_pos - unit_vec_to_obs * obstacle_radius

        #     # Relative position in global coordinates, normalized
        #     closest_dx = np.clip((edge_point_world[0] - agent_pos[0]) / self.max_detect_dis, -1.0, 1.0)
        #     closest_dy = np.clip((edge_point_world[1] - agent_pos[1]) / self.max_detect_dis, -1.0, 1.0)

        #     _barricade_states.append((edge_distance, closest_dx, closest_dy, edge_point_world))
        ...
        #     _barricade_states.sort(key=lambda x: x[0])
        #     closest_distance = _barricade_states[0][0]
        #     closest_dx, closest_dy = _barricade_states[0][1], _barricade_states[0][2]
        #     direction = 1
    ...
    def step(self, action):
        ...
        #####################################################################################################
            ...
            #####################################################################################################
            if self.is_in_circle(self.obstacles):
    ...
    #     _delta = 4
    #     x, y = self.agent_pos
    #     if not (110 + _delta <= x <= 310 - _delta and 24 + _delta <= y <= 96 - _delta):
    ...
    def plot_env(self, ax, sample_rate=10):
        ...
        if hasattr(self, "obstacles") and self.obstacles:
            for obs in self.obstacles:
                ax.add_patch(plt.Circle(obs["center"], obs["radius"], color='orange', alpha=0.4, linestyle='--', fill=False))
                ax.text(obs["center"][0], obs["center"][1], "V", color='orange', fontsize=8, ha='center', va='center')
