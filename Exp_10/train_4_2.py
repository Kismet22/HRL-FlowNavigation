...
from env_new import train_env_upper_2_4
...
def train(model_id, render_mode=None, checkpoint_path_high=None, checkpoint_path_low=None):
    ...
    # env = train_env_upper_2_4.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _random_range=random_range, _init_flow_num=flow_seed, _pos_normalize=_is_normalize, _is_test = test_mode, _state_dim=state_dim_mode, 
    # _is_random = true_random, _set_random=random_seed, _switch_range = switch_range, u_range=input_max, v_range=input_max, 
    # _forward_only=_is_head, _obstacle_avoid=True, _theta_mode = theta_mode, _is_switch=switch_mode, u_clip=clip_max, v_clip=clip_max, _near_mode=_near_mode)
    ...
    while hl_time_step <= hl_max_training_timesteps:
        ...
        while not done and t < max_ep_len:
            ...
            if _is_head:
                ...
                lc_target_x, lc_target_y = compute_subgoal(pos=agent_position, angle=high_action, r=2)
