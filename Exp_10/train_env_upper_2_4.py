...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
        ...
        self._last_quadrant_choice = None  # None, "13", or "42"
        self.direction_init = 0
        #################################################
    ...
    def step(self, action):
        ...
        self.moving_speed = sqrt(state_1[0]**2 + state_1[1]**2)
        self.speed = state_1[:3]
        ...
        if self._near_mode:
            ...
            # 相对位置（可选是否转局部坐标）
            vec_center_local = obstacle_pos - agent_pos
            # vec_center_local = R @ (obstacle_pos - agent_pos)  # 如果你需要局部系

            raw_dx, raw_dy = vec_center_local[0], vec_center_local[1]

            # 归一化处理
            if self._pos_normalize:
                closest_dx = raw_dx / self.max_detect_dis
                closest_dy = raw_dy / self.max_detect_dis
                _dis_2_barricade = center_distance
            else:
                closest_dx = raw_dx
                closest_dy = raw_dy
                _dis_2_barricade = center_distance
            ...
            # 逆时针角度
            ccw_angle = signed_angle
            # 顺时针角度 = 2π - 逆时针
            cw_angle = 2 * np.pi - ccw_angle
            if not hasattr(self, "direction_init") or self.direction_init == 0:
                # ------------------ 第一次：选择较小角度方向 ------------------
                if ccw_angle <= cw_angle:
                    self.direction_init = +1   # +1 表示逆时针
                    roll_angle = ccw_angle
                else:
                    self.direction_init = -1   # -1 表示顺时针
                    roll_angle = cw_angle
            else:
                # ------------------ 后续：沿固定方向取角度 ------------------
                if self.direction_init == +1:   # 已锁定逆时针
                    roll_angle = ccw_angle
                else:                           # 已锁定顺时针
                    roll_angle = cw_angle
            barricade_state = np.array([closest_dx, closest_dy], dtype=np.float32)
            angle_norm = roll_angle / (2 * np.pi)
            direction = self.direction_init
            # ------------------ 偏离检测 ------------------
            deviation_flag = False
            roll_success_flag = False
            if angle_norm > 0.75:
                deviation_flag = True
            if angle_norm < 0.025:
                roll_success_flag = True
        else:
        ...
        if self._near_mode:
            # ---------- Step 1: Failure checks ----------
            failure = False
            if _dis_2_barricade > 48:
                failure = True
                _reward = -200
                print(colored(f"**** Episode Finished At: {self.step_counter} **** Too Far.", 'red'))
            elif self.is_in_circle(self.circles):
                self._collision_count += 1
                failure = True
                _reward = -200
                print(colored(f"**** Episode Finished At: {self.step_counter} **** Collision.", 'yellow'))
            elif deviation_flag:
                failure = True
                _reward = -200
                print(colored(f"**** Episode Finished At: {self.step_counter} **** Deviation.", 'red'))
            elif self.step_counter >= self.t_step_max:
                failure = True
                _reward = -50
                print(colored(f"**** Episode Finished At: {self.step_counter} **** Timeout.", 'blue'))
            if failure:
                _truncated = True

            # ---------- Step 2: Success checks ----------
            elif self.is_reach_target():
                _terminated = True
                _reward = 200
                print(colored(f"**** Episode Finished At: {self.step_counter} **** SUCCESS.", 'green'))

            # ---------- Step 3: Normal step reward ----------
            else:
                # 绕行阶段：沿绕行方向 + 避障
                angle_reward = 25 * (self.old_angle - angle_norm)
                avoid_penalty = 50 * (_dis_2_barricade - self._old_dis_2_barricade)  
                if not roll_success_flag:
                    ideal_min, ideal_max = 24, 32
                    _reward = -2 * self.dt + angle_reward
                    if _dis_2_barricade < ideal_min:
                        # 太近：奖励远离
                        _reward += avoid_penalty
                    elif _dis_2_barricade > ideal_max:
                        # 太远：奖励靠近
                        _reward -= avoid_penalty                        
                    else: 
                        _reward = -2 * self.dt + angle_reward + avoid_penalty
                else:
                    # 绕行完成：接近目标 + 避障
            ...
            if self.is_out_of_bounds():
                _out_bound = True
                _truncated = True
                _reward = -200 if self._near_mode else -500
            ...
            elif self._near_mode and _dis_2_barricade > 56:
                _out_bound = True
                _truncated = True
                _reward = -100
                print(colored(f"**** Episode Finished At: {self.step_counter} **** Too Far.", 'red'))
            
            elif self._near_mode and deviation_flag:
                _out_bound = True
                _truncated = True
                _reward = -100
                print(colored(f"**** Episode Finished At: {self.step_counter} **** Deviation.", 'red'))
            ...
            # 5. 正常步奖励
                    ...
                    # ----------------- Near-mode immediate termination -----------------
                    if self._near_mode and direction_switch:
                        _truncated = True
                        _reward = -200
                        print(colored(f"**** Episode terminated: Direction Switch in Near Mode ****", 'red'))

                    # ----------------- Penalty & Reward -----------------
                    if not _truncated:  # 仅当未提前终止时计算正常 reward
                        if self._near_mode:
                            avoid_penalty = 25 * (_dis_2_barricade - self._old_dis_2_barricade)
                            target_reward = 10 * (self.d_2_target - d)
                            angle_reward = 20 * (self.old_angle - angle_norm)
                            roll_swich_punish = -20 * abs(direction - self.old_direction) * abs(self.old_direction) * abs(direction)
                        else:
                            avoid_penalty = 25 * (_dis_2_barricade - self._old_dis_2_barricade)
                            target_reward = 10 * (self.d_2_target - d)
                        ...
                        # ----------------- Mixed Reward by observation_dim -----------------
                        if self.observation_dim in [6, 14]:
                            _reward = -2 * self.dt + target_reward
                        elif self.observation_dim in [8, 16]:
                            _reward = -2 * self.dt + target_reward + avoid_penalty
                        elif self.observation_dim in [10, 18]:
    ...
    def generate_start_and_target_pair(self, circle_center=(220,64), circle_radius=32, boundary_margin=16.0,min_angle_deg=90, max_angle_deg=170, max_tries=1000):
        ...
        for _ in range(max_tries):
            ...
            theta = random.uniform(0, 2*np.pi)
            cx = cx0 + r * np.cos(theta)
            cy = cy0 + r * np.sin(theta)
            center = np.array([cx, cy])
            self._roll_center = center

            # 优先尝试与上一次不同的组合
            if self._last_quadrant_choice != "13":
            ...
            if min_angle_deg <= angle <= max_angle_deg:
                # 只有生成成功后才更新 last_choice
                self._last_quadrant_choice = candidate_choice
    ...
    def reset(self):
        ...
        if self._near_mode:
            ...
            direction = self.direction_init
            # ------------------ 偏离检测 ------------------
            # 如果 roll_angle 比初始角度大很多，说明绕行反而越绕越远
            # deviation_flag = False
            # if angle_norm > 0.75:
            #     deviation_flag = True
        else:
        ...
        if self._near_mode:
            if self.observation_dim == 10:
                # no pressure + barricate sensor + roll direction guidance
                self.state = np.hstack([state_1[:6], barricade_state, self.direction_init, angle_norm]).astype(np.float32)
            if self.observation_dim == 18:
                self.state = np.hstack([state_1, barricade_state, self.direction_init, angle_norm]).astype(np.float32)
        else:
            if self.observation_dim == 6:
                # no pressure
                self.state = state_1[:6]
        ...
        print(colored(f"DEBUG reset {self.step_counter}: near_mode={self._near_mode}, "f"old_dis={self._old_dis_2_barricade}, max_detect_dis={self.max_detect_dis}, "f"init_angle={angle_norm:.3f}, direction={direction}","magenta"))
    ...
    def is_in_circle(self, circles):
        ...
        # Hitting virtual obstacle (roll_center)
        if hasattr(self, "_roll_center") and self._roll_center is not None:
            virtual_radius = getattr(self, "_roll_radius", self.window_r/2)
            distance = np.linalg.norm(self.agent_pos - self._roll_center)
            if distance <= (a + virtual_radius):
                return True
