...
class foil_env:
        ...
        #####################################################################################################
            ...
            #####################################################################################################
                ...
                if self.observation_dim == 16:
                    _reward = time_punish + target_reward + 1.6 * avoid_penalty
                ########################
    ...
    def plot_env(self, ax_env, ax_pressure=None, sample_rate=10):
        ...
        if flow_x is not None and flow_y is not None:
            ...
            im = ax.pcolormesh(X, Y, speed_clipped, cmap=cmap, norm=norm, shading='auto', alpha=0.6, zorder=1)
